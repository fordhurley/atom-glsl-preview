...
module.exports = class GlslPreviewView {
  ...
  renderShader() {
    if (/^\s*uniform\s+vec2\s+u_resolution/m.test(this.source)) {
      this.shader.setUniform("u_resolution", this.shader.getResolution());
    }
    if (/^\s*uniform\s+float\s+u_time/m.test(this.source)) {
      this.shader.setUniform("u_time", performance.now()/1000);
    }
    this.shader.render();
  }
