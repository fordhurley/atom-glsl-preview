...
const StatusOverlay = require("./status-overlay");
...
module.exports = class GlslPreviewView {
  constructor({editorId}) {
    ...
    this.shader.onShaderError = (errors) => {
      const translatedErrors = translateShaderErrors(
        errors,
        this.source,
        this.compiledSource,
        this.shader.prefix
      );
      this.showErrors(translatedErrors);
    };
    ...
    this.element.appendChild(this.statusOverlay.domElement);
  ...
  updateShader(source) {
    ...
    try {
      this.clearMarkers();
      this.compiledSource = glslify.compile(this.source, {
    ...
    this.shader.setShader(this.compiledSource, Config.includeDefaultUniforms());
  }
  ...
  recordVideoStream(seconds, framesPerSecond) {
    this.getSavePath(".mp4", (outputPath) => {
      ...
      const renderFrame = () => {
        ...
        const framesEncodedPerSecond = (frameNum + 1) / elapsedSeconds;
        const fpsString = framesEncodedPerSecond.toFixed(1);
        const status = `Recording video (frame ${frameNum} / ${numFrames}, ${fpsString} FPS)`;

        this.statusOverlay.setProgress(frameNum / numFrames);
        this.statusOverlay.setMessage(status);
      ...
      encoder.run().then(() => {
        const elapsedSeconds = (performance.now() / 1000) - startTimeSeconds;
        const elapsedString = elapsedSeconds.toFixed(2).replace(/\.?0*$/, "") + " seconds";
        atom.notifications.addSuccess("Video saved", {
          detail: `Saved to: ${outputPath} in ${elapsedString}.`,
        });
      }).error((e) => {
        ...
        this.statusOverlay.setProgress(0);
