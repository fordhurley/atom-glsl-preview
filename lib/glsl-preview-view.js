...
const {debounce, defer} = require("underscore");
...
module.exports = class GlslPreviewView {
  ...
  recordVideoStream(seconds, framesPerSecond) {
    ...
    const frameStream = encoder.input({f: "image2pipe", vcodec: "mjpeg"});

    let outputPath = this.lastSavePath;
    if (!outputPath) {
      if (this.editor) {
        outputPath = this.editor.getPath();
      }
      const {dir, name} = path.parse(outputPath);
      outputPath = path.format({dir, name, ext: ".mp4"});
    }
    encoder.output(outputPath, {f: "mp4", r: framesPerSecond});
    ...
    atom.notifications.addInfo("Recording video", {
      detail: `This should take ~${estimatedSeconds} seconds.`,
      dismissable: false,
    });
    ...
    encoder.run().then(() => {
      ...
      atom.notifications.addSuccess("Video saved", {
        detail: `Saved to: ${outputPath} in ${elapsedSeconds}
        seconds (${elapsedSeconds / numFrames} seconds/frame)`,
        dismissable: true,
      }); // TODO: dispose?
    }).error((e) => {
      atom.notifications.addError("Video recording failed", {
        detail: e,
        dismissable: true,
      }); // TODO: dispose?
    }).finally(() => {
